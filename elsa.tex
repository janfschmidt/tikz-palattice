...
\sisetup{locale=DE, range-units=single, range-phrase=\,-\,, detect-weight=true, detect-family=true}
...
\begin{lattice}[][\scale]
\def\quadl{0.2}
\setelementcolor{dipole}{blue!60!cyan!90}[blue!60!cyan!90]
...
%-----------------------------
\begin{scope}
\setelementcolor{dipole}{green!50!black}[green!50!black]
\goto{BoosterInj}
%\marker{Start}[0.5]
...
{Synchrotronlicht\\{Diagnoseplatz}};
...
\setlinecolor{drift}{\photonbeam}
\drift{16}
\beamdump{Photonenkamera}{0.4}[0.2]
\end{scope}

%title
\node[mediumlabel, anchor=east, yshift=1.5cm] at (CBlabel) {Crystal Barrel};
\node[biglabel, yshift=3.8cm,xshift=-3cm, text width=6cm] at (CBlabel)
...
\beamdump{}{0.4}
  % title
  \node[smalllabel, anchor=north, yshift=-5mm, text width=4cm] at (NEWSFlabel)
